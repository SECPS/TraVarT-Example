...
public class TraVarTTransformer<T, V> {
	public void transform(final IPlugin<T> input, final IPlugin<V> output, final Path basePath)
		...
		final Path inputFolderPath = Paths.get(basePath.toString(), INPUT_RESOURCE_FOLDER,
				input.getName().toLowerCase());
		final Set<Path> inputPathSet = TraVarTUtils.getPathSet(inputFolderPath,
		...
		final Path outputBasePath = Paths.get(basePath.toString(), OUTPUT_RESOURCE_FOLDER,
				output.getName().toLowerCase());
		if (!outputBasePath.toFile().exists()) {
			outputBasePath.toFile().mkdir();
		}
		final UVLWriter uvlWriter = new UVLWriter();
		final FeatureModelStatistics featureModelStatistics = FeatureModelStatistics.getInstance();
		...
		for (final Path inp : inputPathSet) {
			...
			final Path outputPath = Paths
					.get(outputBasePath + "/" + fileName + output.getSupportedFileExtensions().get(0));
